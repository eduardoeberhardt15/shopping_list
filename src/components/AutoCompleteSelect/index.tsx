...
import productsController, {
  products,
} from "../../database/controllers/products";
...
const AutoCompleteSelect = ({ addTodo, data, listId }: StateProps) => {
  ...
  async function addNewTodo(value: number, label: string, favorite: boolean) {
    ...
    if (!founded) {
      ...
      addTodo({ id, name: label, list: listId, complete: false, favorite });
    }
  ...
  const newProduct = async () => {
    const id = await productsController().insert({
      name: searchText,
      category: 11,
      id: 0,
    });
          ...
          >
            {dataList.map((item, index) => (
              <TouchableOpacity
                key={index}
                style={styles.item}
                onPress={() => addNewTodo(item.id, item.name, item.favorite)}
              >
            ...
            {dataList.length === 0 && searchText.length >= 3 && (
              <TouchableOpacity style={styles.item} onPress={newProduct}>
                <Text style={styles.text}>
                  {searchText}
                  <Text style={styles.newItem}> - novo item</Text>
                </Text>
              </TouchableOpacity>
            )}
          </ScrollView>
