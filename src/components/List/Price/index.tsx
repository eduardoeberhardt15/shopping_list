...
import { formatStringToMoney, formatMoneyToDecimal } from '../../../extras/money';
...
const Price:React.FC<Props> = ({itemId, listId, price:parentPrice}) => {
    ...
    const [price, setPrice] = useState(parentPrice ? 
        `${formatStringToMoney(Number(parentPrice).toFixed(2))}` 
        : "");
    ...
    const handleFormatMoney = (value:string) =>{
        ...
        let money = formatStringToMoney(value);
    ...
    return(
        ...
        value={price} onChangeText={handleFormatMoney}/>
    );
}
